...
import {
  ...
  RevisionInfo,
  Status,
} from "../typescript_fetch/index.ts";
import { buildQueryString, numbersToKanji } from "../utils/helper.ts";
import logger from "../utils/logger.ts";
import { validateRequest } from "../utils/zod-schemas/common-schema.ts";
import {
  RequestLawDataParams,
  RequestLawDataParamsSchema,
} from "../utils/zod-schemas/request-law-data-params-schema.ts";
...
import { getEraLabel, getTypeByFind } from "../law/law.ts";
...
export const getLawData = async ({
  ...
  const url = `${Deno.env.get("API_URL")}/lawdata?${queryString}`;
  try {
